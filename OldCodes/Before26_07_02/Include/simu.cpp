...
inline myfloat funcReynoldsNbSphereInAirMuVars(myfloat r, myfloat vel){
...
void checkArguments(int argc, char** args){
    ...
    assert(argc >= 7); // TODO need to do this even in the non debug versions
...
void initCleanLagSimu(int argc, char** args, LagSimuParams_t* simu){
    ...
    double µs_to_drop_std_dev = atof(args[a++]);
    updateNormalDropletDistributionInletTime(simu->µs_to_drop, µs_to_drop_std_dev);
    simu->dt = atof(args[a++]);
    assert(simu->dt < 1);
...
void mainLoopCleanLagSimu(LagSimuParams_t* simu, uint32_t nb_frame_simul){
    ...
    while (simu->current_saved < nb_to_save && while_i++ < nb_frame_simul){
        if(simu->t_ind == simu->next_drop_att_ind || simu->t_ind > simu->next_drop_att_ind){
            ...
            std::cout << simu->t_ind << " " << simu->next_drop_att_ind << ", time=" << simu->t_ind*dt << ", µs_to_drop=" << µs_to_drop << std::endl << std::flush;
        } 

        for(int i = 0; i < simu->current_max_index; i++){
            #ifdef INCLUDE_ACCELERATION_AND_DRAG
                myfloat acceleration = GRAVITY*1e-6  - 0.5*RHO_AIR*v_vec->vals[i] * fabs(v_vec->vals[i]) * // MYPOW(v_vec->vals[i], 2)*
